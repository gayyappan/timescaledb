 ...
 *
 * The caller must hold AccessExclusiveLock on the raw hypertable and must have
 * already made the catalog change that turns tracking off for it.  PostgreSQL
 * runs XACT_EVENT_COMMIT (xact.c CommitTransaction) after the commit record is
 * written but *before* RESOURCE_RELEASE_LOCKS, so the hypertable is still
 * locked when the free runs and no writer can be inside the tracker.
 *
 * Attaching here rather than in the callback is deliberate: dsa_attach_in_place
 * and dshash_attach can throw, and by XACT_EVENT_COMMIT the transaction is past
...
ts_tenant_tracker_remove_at_commit(int32 hypertable_id)
{
	...
	if (!tenant_tracker_attach())
	{
		return; /* loader not present -> nothing was ever tracked */
	}
 ...
 * any other end state.  On abort nothing was freed, so there is nothing to undo
 * -- the tracker simply stays, matching the catalog that just rolled back.
 */
static void
tenant_tracker_removal_xact_callback(XactEvent event, void *arg)
{
	...
	switch (event)
	{
		case XACT_EVENT_PRE_PREPARE:
			...
			/*
			 * A prepared transaction's locks are handed to the gxact and
			 * released by whichever backend runs COMMIT PREPARED, so there is
			 * no later moment at which we still hold AccessExclusiveLock and
			 * know the transaction committed.  Refuse rather than free at a
			 * point where a writer could race us.
			 */
			ereport(ERROR,
		...
		case XACT_EVENT_PARALLEL_ABORT:
			/* Parallel workers never run this DDL, so those cases only guard
			 * against the queue outliving the transaction that filled it. */
			pending_removals_reset();
			break;
...
/*
 * ROLLBACK TO SAVEPOINT undoes the catalog change that queued the removal, so
 * drop the entries that subtransaction added.  Note the hypertable lock is not
 * released here -- subtransaction locks are reassigned to the parent and held
 * to top-level end -- so a later disable in the same transaction still runs
 * with no writers around.
 *
 * SUBXACT_EVENT_COMMIT_SUB is deliberately not handled: releasing a savepoint
 * does not make the change durable, and the top-level callback above still
 * decides.
 */
static void
