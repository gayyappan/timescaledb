...
#include <postgres.h>
#include <access/nbtree.h> //validate what we are using here TODO
...
static BTStack ts_bt_search_insert(Relation rel, TupleDesc rel_tupdesc, BTInsertState insertstate);
static TransactionId ts_bt_check_unique(Relation rel, TupleDesc rel_tupdesc,
										BTInsertState insertstate, Relation heapRel,
										IndexUniqueCheck checkUnique, bool *is_unique,
										uint32 *speculativeToken);
static OffsetNumber ts_bt_findinsertloc(Relation rel, TupleDesc rel_tupdesc,
										BTInsertState insertstate, bool checkingunique,
										BTStack stack, Relation heapRel);
static void _bt_stepright(Relation rel, BTInsertState insertstate, BTStack stack);
static void _bt_insertonpg(Relation rel, BTScanInsert itup_key, Buffer buf, Buffer cbuf,
						   BTStack stack, IndexTuple itup, Size itemsz, OffsetNumber newitemoff,
						   int postingoff, bool split_only_page);
static Buffer _bt_split(Relation rel, BTScanInsert itup_key, Buffer buf, Buffer cbuf,
						OffsetNumber newitemoff, Size newitemsz, IndexTuple newitem,
						IndexTuple orignewitem, IndexTuple nposting, uint16 postingoff);
static void _bt_insert_parent(Relation rel, Buffer buf, Buffer rbuf, BTStack stack, bool is_root,
							  bool is_only);
static Buffer _bt_newroot(Relation rel, Buffer lbuf, Buffer rbuf);
static inline bool _bt_pgaddtup(Page page, Size itemsize, IndexTuple itup, OffsetNumber itup_off,
								bool newfirstdataitem);
static void _bt_vacuum_one_page(Relation rel, Buffer buffer, Relation heapRel);

/*
 *	ts_compress_uk_doinsert() -- Handle insertion of a single index tuple in the tree.
 *
 ...
 */
bool
ts_compress_uk_doinsert(Relation rel, TupleDesc rel_tupdesc, IndexTuple itup,
						IndexUniqueCheck checkUnique, Relation heapRel)
{
	...
	itup_key = compress_uk_mkscankey(rel, rel_tupdesc, itup);
...
search:
	 ...
	 */
	stack = ts_bt_search_insert(rel, rel_tupdesc, &insertstate);
	...
	if (checkUnique != UNIQUE_CHECK_EXISTING)
	{
		 ...
		 */
		newitemoff =
			ts_bt_findinsertloc(rel, rel_tupdesc, &insertstate, checkingunique, stack, heapRel);
		_bt_insertonpg(rel,
					   itup_key,
					   insertstate.buf,
					   InvalidBuffer,
...
/*
 *	ts_bt_search_insert() -- _bt_search() wrapper for inserts
 *
 ...
 */
static BTStack
ts_bt_search_insert(Relation rel, TupleDesc rel_tupdesc, BTInsertState insertstate)
{
	...
	if (RelationGetTargetBlock(rel) != InvalidBlockNumber)
	{
		...
		if (ConditionalLockBuffer(insertstate->buf))
		{
			 ...
			 * inserter.)
			 */
			if (P_RIGHTMOST(lpageop) && P_ISLEAF(lpageop) && !P_IGNORE(lpageop) &&
				PageGetFreeSpace(page) > insertstate->itemsz &&
				PageGetMaxOffsetNumber(page) >= P_HIKEY &&
				ts_bt_compare(rel, rel_tupdesc, insertstate->itup_key, page, P_HIKEY) > 0)
			{
				/*
...
/*
 *	ts_bt_check_unique() -- Check for violation of unique index constraint
 *
...
ts_bt_check_unique(Relation rel, TupleDesc rel_tupdesc, BTInsertState insertstate, Relation heapRel,
	...
	*is_unique = true;
	...
	for (;;)
	{
		...
		if (offset <= maxoff)
		{
			 ...
			 * ts_bt_compare() directly when there are no duplicates, as long as
			...
			if (nbuf == InvalidBuffer && offset == insertstate->stricthigh)
			{
				...
				Assert(ts_bt_compare(rel, rel_tupdesc, itup_key, page, offset) < 0);
				break;
			}
			 ...
			 * many killed items with the same key; running ts_bt_compare() on
			 * each killed item gets expensive.  Just advance over killed
			 * items as quickly as we can.  We only apply ts_bt_compare() when
			 * we get to a non-killed item.  We could reuse the bounds to
			 * avoid ts_bt_compare() calls for known equal tuples, but it
			 * doesn't seem worth it.  Workloads with heavy update activity
			 * tend to have many deduplication passes, so we'll often avoid
			 * most of those comparisons, too (we call ts_bt_compare() when the
			...
			if (inposting || !ItemIdIsDead(curitemid))
			{
				...
				if (!inposting)
				{
					...
					if (ts_bt_compare(rel, rel_tupdesc, itup_key, page, offset) != 0)
				...
				else if (table_index_fetch_tuple_check(heapRel, &htid, &SnapshotDirty, &all_dead))
				{
					 ...
					 */
					xwait = (TransactionIdIsValid(SnapshotDirty.xmin)) ? SnapshotDirty.xmin :
																		 SnapshotDirty.xmax;
					...
					if (TransactionIdIsValid(xwait))
					{
						if (nbuf != InvalidBuffer)
							_bt_relbuf(rel, nbuf);
						/* Tell ts_compress_uk_doinsert to wait... */
					 ...
					 * violation.
					 */
					CheckForSerializableConflictIn(rel,
												   NULL,
...

					{
						...
						ereport(ERROR,
						 ...
						 (!inposting ||
						  (prevalldead && curposti == BTreeTupleGetNPosting(curitup) - 1)))
				{
					/*
		...
		else
		{
			...
			if (P_RIGHTMOST(opaque))
				break;
			highkeycmp = ts_bt_compare(rel, rel_tupdesc, itup_key, page, P_HIKEY);
			Assert(highkeycmp <= 0);
			if (highkeycmp != 0)
				break;
...
/*
 *	ts_bt_findinsertloc() -- Finds an insert location for a tuple
 *
 ...
 */
static OffsetNumber
ts_bt_findinsertloc(Relation rel, TupleDesc rel_tupdesc, BTInsertState insertstate,
					bool checkingunique, BTStack stack, Relation heapRel)
{
	BTScanInsert itup_key = insertstate->itup_key;
	...
	if (itup_key->heapkeyspace)
	{
		...
		if (checkingunique)
		{
			...
			for (;;)
			{
				...
				if (P_RIGHTMOST(lpageop) ||
					ts_bt_compare(rel, rel_tupdesc, itup_key, page, P_HIKEY) <= 0)
					break;
		...
		if (PageGetFreeSpace(page) < insertstate->itemsz)
		{
			...
			if (itup_key->allequalimage && BTGetDeduplicateItems(rel) &&
				(!checkingunique || uniquedup) && PageGetFreeSpace(page) < insertstate->itemsz)
			{
				_bt_dedup_one_page(rel,
								   insertstate->buf,
								   heapRel,
								   insertstate->itup,
								   insertstate->itemsz,
 ...
 */
static void
_bt_insertonpg(Relation rel, BTScanInsert itup_key, Buffer buf, Buffer cbuf, BTStack stack,
			   IndexTuple itup, Size itemsz, OffsetNumber newitemoff, int postingoff,
			   bool split_only_page)
{
	...
	Assert(!P_ISLEAF(lpageop) ||
		   BTreeTupleGetNAtts(itup, rel) == IndexRelationGetNumberOfAttributes(rel));
	Assert(P_ISLEAF(lpageop) ||
		   BTreeTupleGetNAtts(itup, rel) <= IndexRelationGetNumberOfKeyAttributes(rel));
	...
	else
	{
		 ...
		 * may be used by a future inserter within ts_bt_search_insert().
		 */
		 ...
		 */
		if (BlockNumberIsValid(blockcache) && _bt_getrootheight(rel) >= BTREE_FASTPATH_MIN_LEVEL)
 ...
 */
static Buffer
_bt_split(Relation rel, BTScanInsert itup_key, Buffer buf, Buffer cbuf, OffsetNumber newitemoff,
		  Size newitemsz, IndexTuple newitem, IndexTuple orignewitem, IndexTuple nposting,
		  uint16 postingoff)
{
	Buffer rbuf;
	Page origpage;
	...
	OffsetNumber afterleftoff, afterrightoff, minusinfoff;
	OffsetNumber origpagepostingoff;
	OffsetNumber maxoff;
	OffsetNumber i;
	bool newitemonleft, isleaf, isrightmost;
	 ...
	 * case.
	 */
	firstrightoff = _bt_findsplitloc(rel, origpage, newitemoff, newitemsz, newitem, &newitemonleft);
	...
	{
		Assert(isleaf);
		Assert(ItemPointerCompare(&orignewitem->t_tid, &newitem->t_tid) < 0);
	...
	Assert(BTreeTupleGetNAtts(lefthighkey, rel) <= IndexRelationGetNumberOfKeyAttributes(rel));
	Assert(itemsz == MAXALIGN(IndexTupleSize(lefthighkey)));
	if (PageAddItem(leftpage, (Item) lefthighkey, itemsz, afterleftoff, false, false) ==
		InvalidOffsetNumber)
		elog(ERROR,
	...
	if (!isrightmost)
	{
		...
		if (PageAddItem(rightpage, (Item) righthighkey, itemsz, afterrightoff, false, false) ==
			InvalidOffsetNumber)
		{
	...
	if (RelationNeedsWAL(rel))
	{
		...
		XLogRegisterBufData(1,
							...
							((PageHeader) rightpage)->pd_special -
...
_bt_insert_parent(Relation rel, Buffer buf, Buffer rbuf, BTStack stack, bool is_root, bool is_only)
{
	...
	else
	{
		...
		if (stack == NULL)
		{
			 ...
			 * rejected within ts_bt_search_insert() when the rightmost leaf
		...
		if (pbuf == InvalidBuffer)
			ereport(ERROR,
					(errcode(ERRCODE_INDEX_CORRUPTED),
					 errmsg_internal("failed to re-find parent key in index \"%s\" for split pages "
									 "%u/%u",
									 RelationGetRelationName(rel),
									 bknum,
									 rbknum)));
 ...
 *		stack.  For example, the checkingunique ts_compress_uk_doinsert() case may
...
_bt_newroot(Relation rel, Buffer lbuf, Buffer rbuf)
{
	...
	if (PageAddItem(rootpage, (Item) right_item, right_item_sz, P_FIRSTKEY, false, false) ==
		InvalidOffsetNumber)
		elog(PANIC,
			 ...
			 bool newfirstdataitem)
{
	...
	if (unlikely(PageAddItem(page, (Item) itup, itemsize, itup_off, false, false) ==
				 InvalidOffsetNumber))
		return false;
