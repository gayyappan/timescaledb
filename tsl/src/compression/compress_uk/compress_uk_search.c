...
static OffsetNumber ts_bt_binsrch(Relation rel, TupleDesc rel_tupdesc, BTScanInsert key,
								  Buffer buf);
...
static int _bt_setuppostingitems(BTScanOpaque so, int itemIndex, OffsetNumber offnum,
								 ItemPointer heapTid, IndexTuple itup);
static inline void _bt_savepostingitem(BTScanOpaque so, int itemIndex, OffsetNumber offnum,
...
_bt_drop_lock_and_maybe_pin(IndexScanDesc scan, BTScanPos sp)
{
	...
	if (IsMVCCSnapshot(scan->xs_snapshot) && RelationNeedsWAL(scan->indexRelation) &&
		!scan->xs_want_itup)
	{
...
/*
 *	ts_bt_search() -- Search the tree for a particular scankey,
 ...
 */
BTStack
ts_bt_search(Relation rel, TupleDesc rel_tupdesc, BTScanInsert key, Buffer *bufP, int access,
			 Snapshot snapshot)
{
	...
	*bufP = _bt_getroot(rel, access);
	...
	for (;;)
	{
		 ...
		 *
		 * In write-mode, allow ts_bt_moveright to finish any incomplete splits
		...
		*bufP = ts_bt_moveright(rel,
		 ...
		 */
		offnum = ts_bt_binsrch(rel, rel_tupdesc, key, *bufP);
	...
	if (access == BT_WRITE && page_access == BT_READ)
	{
		 ...
		 */
		*bufP = ts_bt_moveright(rel, rel_tupdesc, key, *bufP, true, stack_in, BT_WRITE, snapshot);
	}
...
/*
 *	ts_bt_moveright() -- move right in the btree if necessary.
 *
 ...
 */
Buffer
ts_bt_moveright(Relation rel, TupleDesc rel_tupdesc, BTScanInsert key, Buffer buf, bool forupdate,
				BTStack stack, int access, Snapshot snapshot)
{
	Page page;
	BTPageOpaque opaque;
	int32 cmpval;
	 ...
	 * ts_bt_search has found an initial leaf page.)
	 *
	...
	for (;;)
	{
		...
		if (P_IGNORE(opaque) || ts_bt_compare(rel, rel_tupdesc, key, page, P_HIKEY) >= cmpval)
		{
			/* step right one page */
...
/*
 *	ts_bt_binsrch() -- Do a binary search for a key on a particular page.
 *
 * On a leaf page, ts_bt_binsrch() returns the OffsetNumber of the first
 * key >= given scankey, or > scankey if nextkey is true.  (NOTE: in
 * particular, this means it is possible to return a value 1 greater than the
 * number of keys on the page, if the scankey is > all keys on the page.)
 *
 * On an internal (non-leaf) page, ts_bt_binsrch() returns the OffsetNumber
 * of the last key < given scankey, or last key <= given scankey if nextkey
 * is true.  (Since ts_bt_compare treats the first data key of such a page as
 ...
 * the given page.  ts_bt_binsrch() has no lock or refcount side effects
 * on the buffer.
 */
static OffsetNumber
ts_bt_binsrch(Relation rel, TupleDesc rel_tupdesc, BTScanInsert key, Buffer buf)
{
	Page page;
	BTPageOpaque opaque;
	...
	/* scantid-set callers must use ts_bt_binsrch_insert() on leaf pages */
	...
	while (high > low)
	{
		...
		result = ts_bt_compare(rel, rel_tupdesc, key, page, mid);
	 ...
	 * There must be one if ts_bt_compare() is playing by the rules.
	 */
...
/*
 *
 *	ts_bt_binsrch_insert() -- Cacheable, incremental leaf page binary search.
 *
 ...
 */
OffsetNumber
ts_bt_binsrch_insert(Relation rel, TupleDesc rel_tupdesc, BTInsertState insertstate)
{
 ...
 *
 * Helper routine for ts_bt_binsrch_insert().
 *
...
_bt_binsrch_posting(BTScanInsert key, Page page, OffsetNumber offnum)
{
	 ...
	 * to be able to relocate a non-pivot tuple using ts_bt_binsrch_insert().)
	 */
	itemid = PageGetItemId(page, offnum);
	itup = (IndexTuple) PageGetItem(page, itemid);
	 ...
	 * to ts_bt_binsrch_insert() caller by returning -1, a sentinel value.  A
	 * second call to ts_bt_binsrch_insert() can take place when its caller has
...
/*----------
 *	ts_bt_compare() -- Compare insertion-type scankey to tuple on a page.
 *
 ...
 */
int32
ts_bt_compare(Relation rel,
			  TupleDesc itupdesc, // descr for key tuple in leaf
			  BTScanInsert key, Page page, OffsetNumber offnum)
{
	...
	for (int i = 1; i <= ncmpkey; i++)
	{
		...
		else
		{
			 ...
			 * ts_bt_compare as comparing the scankey to the index item, we have
...
/*
 *	ts_bt_first() -- Find the first item in a scan.
 *
 ...
 *  from line 853 to 1444 */
bool
ts_bt_first(IndexScanDesc scan, TupleDesc rel_tupdesc, ScanDirection dir)
{
	...
	for (i = 0; i < keysCount; i++)
	{
		...
		{
			// TODO when do we get into this code ??????????????
			/*
		...
		else
		{
			 ...
			 */
			// TODO fix this code !!!!!!!!!!!!!!!!!!!!!!!
			// We have to init with scaninfo for the underlying type of the tupledesc stored
			FmgrInfo procinfo;
			/* get comparison function for the type. rel_tupdesc has
			 * descriptor for entries in the btree leaf nodes
			 * this is expensive, we should cache this someplace TODO !!!!
			 */
			Form_pg_attribute attr = TupleDescAttr(rel_tupdesc, cur->sk_attno);
			Oid attype = attr->atttypid;
			TypeCacheEntry *tce = lookup_type_cache(attype, TYPECACHE_BTREE_OPFAMILY);
			Oid order_proc_oid = get_opfamily_proc(tce->btree_opf, attype, attype, BTORDER_PROC);
			/* where should this be alloced TODOin rd_amcache and
			 * use rd_indexcxt. Should save this in State and then retrieve
			 * from there. TODO !!!!!!!!!!!!!!!!!
			 */
			if (!RegProcedureIsValid(order_proc_oid))
				elog(ERROR,
	 ...
	 */
	stack = ts_bt_search(rel, rel_tupdesc, &inskey, &buf, BT_READ, scan->xs_snapshot);
	...
	offnum = ts_bt_binsrch(rel, rel_tupdesc, &inskey, buf);
...
/*
 *	ts_bt_next() -- Get the next item in a scan.
 *
 ...
 */
bool
ts_bt_next(IndexScanDesc scan, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	BTScanPosItem *currItem;
		...
		while (offnum <= maxoff)
		{
			...
			if (_bt_checkkeys(scan, itup, indnatts, dir, &continuescan))
			{
				...
				else
				{
					 ...
					 * TID
					 */
					tupleOffset = _bt_setuppostingitems(so,
														itemIndex,
														offnum,
 ...
 */
static int
_bt_setuppostingitems(BTScanOpaque so, int itemIndex, OffsetNumber offnum, ItemPointer heapTid,
					  IndexTuple itup)
{
	BTScanPosItem *currItem = &so->currPos.items[itemIndex];
 ...
 */
static inline void
_bt_savepostingitem(BTScanOpaque so, int itemIndex, OffsetNumber offnum, ItemPointer heapTid,
					int tupleOffset)
{
	BTScanPosItem *currItem = &so->currPos.items[itemIndex];
 ...
 *
 * This is used by ts_bt_first() to set up a scan when we've determined
 * that the scan must start at the beginning or end of the index (for
 * a forward or backward scan respectively).  Exit conditions are the
 * same as for ts_bt_first().
 */
static bool
_bt_endpoint(IndexScanDesc scan, ScanDirection dir)
{
	 ...
	 * version of ts_bt_search().  We don't maintain a stack since we know we
	 * won't need it.
	 */
