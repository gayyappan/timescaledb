...
----------------------------------------------------------------------
-- Disabling and the tenant tracker's shared memory
--
...
-- the tracker's shared memory is deferred to commit, so it does not happen
-- here either -- otherwise the restored row below would describe a tracker
...
-- Same, undone by a savepoint rather than a whole transaction.
BEGIN;
SAVEPOINT sp;
ALTER MATERIALIZED VIEW readings_hourly SET (timescaledb.enable_granular_refresh = false);
ALTER TABLE readings SET (timescaledb.cagg_enable_granular_refresh = false);
...
-- frees the tracker, so the cached handle must be dropped: writing through a
-- stale one would touch shared memory that has been handed back to the DSA.
ALTER MATERIALIZED VIEW readings_hourly SET (timescaledb.enable_granular_refresh = false);
ALTER TABLE readings SET (timescaledb.cagg_enable_granular_refresh = false);
:GRC 'readings';
